...
from settings import get_fullscreen, set_high_score
...
def run_game():
    ...
    while running:
        ...
        for extra_score_obj in extra_score_group:
            extra_score_obj.update()
            extra_score_obj.draw(screen)

            if player.rect.colliderect(extra_score_obj.rect):
                score += 20
                extra_score_obj.kill()
                extra_score_obj.sound_effect.play()

            if score >= 3000:
                extra_score_obj.speed = 2
            if score >= 10000:
                extra_score_obj.speed = 4
            if score >= 15000:
                extra_score_obj.speed = 6
            if score >= 20000:
                extra_score_obj.speed = 8
